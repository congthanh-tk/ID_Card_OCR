...
def cropout_unimportant_part(img):
    ...
    # Phần ânh chứa thông tin
    img = img[y:h, x:w]
    return img, pic

# Căt phân nhãn (Họ tênn, Ngày sinh,... không có ID)
def crop_label(img):
    h, w, _ = img.shape
...
def get_main_text(img, box, kernel_height):
    ...
    max_box = max(contour_boxes, key=lambda tup: tup[2] * tup[3]) # Viên lớn nhất
    x, y, w, h = max_box
...
def remove_smaller_area(group, width):
    ...
    for element in group_orig:
        # if element[0] < width/10:
        if element[0] < width/5:
            group.remove(element)
        # elif element[-1] * element[-2] < avg and element[0] < width/5:
        elif element[-1] * element[-2] < avg and element[0] < width/2:
            group.remove(element)
    return group
...
def detect_info(img):
    ...
    orig = img.copy() # Ảnh chỉ chứa phần thông tin
    img, ratio = resize_img_by_height(img) # resize image to height = 500 pixels
    ...
    number_box = (0, 0, img.shape[1], info_list[0][1] + 10) # Cắt từ phần text Họ và tên trở lên
    number_box = get_main_text(img, number_box, 5) # Bo phan khoang trang thua xung quanh text => Tra ve (x, y, x', y')
    number_img = get_img_from_box(orig, ratio, number_box) # Cat phần text chinh cua dong, dua ve kich thuoc goc

    # get name part
    name_box = info_list[0] # Ho va ten box
...
def detect_info_CMTND(img):
    ...
    # get another name part
    another_name_box = info_list[1] # Ho va ten box
    another_name_box = get_name(img, get_main_text(img, another_name_box, 5))
    another_name_img = get_img_from_box(orig, ratio, another_name_box, padding=2)
    another_name_img = cut_blank_part(another_name_img)
    ...
    # get gender_and national part
    gender_and_nationality_box = info_list[3]
    gender_and_nationality_box = get_main_text(img, gender_and_nationality_box, 5)
    gender_n_nation_img = get_img_from_box(orig, ratio, gender_and_nationality_box, padding=2)
    h, w, _ = gender_n_nation_img.shape
    gender_img = gender_n_nation_img[0:h, 0:int(w/3)]
    ...
    country_result = get_text_from_two_lines(img, (x, last_y, x1, y1))
    country_img_list = process_result(orig, ratio, country_result)
    address_box = info_list[5]
    x, y, x1, y1 = address_box
    # y1 = y1 + 200
    last_y = get_last_y(country_result)
    address_img = process_result(orig, ratio, get_two_lines_img(img, (x, last_y, x1, y1)))[0]
    result = get_text_from_two_lines(img, (x, last_y, x1, y1))
    address_img_list = process_result(orig, ratio, result)

    return face, number_img, name_img, another_name_img, dob_img, gender_img, nation_img, country_img, \
        address_img, country_img_list, address_img_list
